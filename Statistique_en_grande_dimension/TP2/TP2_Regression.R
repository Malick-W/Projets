...
path_School = "/users/mmath/wade/Bureau/Data/Statistique_en_grande_dimension/leukemia_small.csv"

leukemia_small= read.csv(path_School, header = F)
...
par(mfrow=c(2,2))
hist(X[Y == 0, j], col = grey(0.8), main = "Histogramme", xlab="Géne 10")
qqnorm(X[Y == 0, j])
qqline(X[Y == 0, j], col = "red")

hist(X[Y == 1, j], col = grey(0.8), main = "Histogramme", xlab="Géne 10")
qqnorm(X[Y == 1, j], main = "Histogramme", xlab="Géne 15")
qqline(X[Y == 1, j], col = "red")

# On peut effectuer le test de Shapiro-Wilk
shapiro.test(X[Y == 1, j])
...
TrainEN0 = glmnet(TrainX, TrainY, alpha=0.1, family="binomial")
...
LambdaEN0 = cv.glmnet(TrainX, TrainY, family="binomial", type.measure="class", alpha=0.1)
...
meilleurLambdaEN0 = LambdaEN0$lambda.min
...
PredEN0 = as.numeric(predict(LambdaEN1$glmnet.fit, TestX, s=meilleurLambdaEN0, type="class"))
...
E0 = sum(abs(TestY - PredEN0))/nTest
...
df_Erreur = data.frame(erreur=round(c(E0,E1,E2,E3,E4,E5),3),
                    row.names=c("ElasticNet_0.1", "Lasso","Ridge", "ElasticNet_0.25","ElasticNet_0.5","ElasticNet_0.75"))
df_Erreur
